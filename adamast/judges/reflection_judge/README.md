...
| [`../../llm/reflection_judge_llm.py`](../../llm/reflection_judge_llm.py) | LLM transport bridge (lives in `adamast/llm/`): adapts adamast's `learning_calls.support_model_call` to the `LLMCall` signature the judge expects. Surfaces parse failures + truncation diagnostics in a `warnings` list |
